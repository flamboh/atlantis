<script lang="ts">
	...
	type ProtocolChartRecord = {
		router: string;
		bucket: TimeBucket<ProtocolStatsBucket>;
	};
	let buckets = $state<ProtocolChartRecord[]>([]);
	...
	function renderChart() {
		...
		const bucketByRouterAndStart: Record<string, ProtocolChartRecord> = {};
	...
	async function loadData(filters: FilterInputs, token: number) {
		...
		try {
			await ensureCachedWindow<ProtocolChartRecord>({
				key: cacheKey,
				requestedRange,
			...
			buckets = readCachedWindow<ProtocolChartRecord>(cacheKey, requestedRange, (record, range) => {

...
import type { AlertFeedAddress, AlertsFeedResponse } from '#lib/types/types.ts';
import type { AlertsFeedQuery, QueryParam } from './driver.ts';
...
function readAddresses(
	db: SqliteClient,
	options: Pick<AlertsFeedQuery, 'tail' | 'sort' | 'limit'>,
	horizonStart: number,
			...
			`
				...
				ranked AS (
					SELECT
						...
						ROW_NUMBER() OVER (
							PARTITION BY address
							ORDER BY window_start DESC, rank ASC
						) AS recent_rank,
				...
				enriched AS (
					SELECT
						ranked.*,
						MAX(CASE WHEN recent_rank = 1 THEN alpha END)
							OVER (PARTITION BY address) AS latest_alpha
					FROM ranked
				)
				SELECT
					address,
					...
					(SELECT MIN(history.window_start)
						...
						WHERE history.address = enriched.address) AS firstSeen,
...
export async function readAlertsFeed(
	datasetId: string,
	query: AlertsFeedQuery
): Promise<AlertsFeedResponse | null> {
	...
	try {
		...
		const result = latestWindow
			? readAddresses(
					db,
					query,
					latestWindow.windowEnd - query.horizonSeconds,
					thresholdHigh,
					thresholdLow
				)
		...
		return {
			...
			horizonSeconds: query.horizonSeconds,
			...result
		};
	} catch {
		if (alertsDbPath) {

...
- `sources`: optional logical source list for nfcapd datasets. `members` are physical top-level directories, and `source_id` is the logical source written to v2 tables. If omitted, each top-level directory is treated as a single-member source. Pipeline v2 copies this membership into SQLite metadata so traffic overview queries can avoid double-counting additive metrics.

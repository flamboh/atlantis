...
`--no-maad`. A selected population is a distinct database product and should
...
The native adapter invokes external `nfdump` with the fixed 15-column
`nfdump-csv-15-v1` contract. Reduction and MAAD run inside Rust. Every input is
bound to its SHA-256 content fingerprint and canonical decoder fingerprint;
ordinary replacement or in-place modification is rejected instead of mixing
products. Synthetic gaps verify continued file absence during publication.

Each SQLite database is bound once to its schema, normalized flow selection,
pipeline timezone, reducer contract, and MAAD setting. Schema, source-layout,

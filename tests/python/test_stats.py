...
def test_stats_insert_round_trip() -> None:
    ...
    stats.insert_port_count_stats_rows(conn, rows['port_count_rows'])
    ...
    assert conn.execute(
        'SELECT average_duration_ms, average_min_ttl, average_max_ttl FROM traffic_stats'
    ).fetchone() == (250.0, 31.0, 64.0)
    ...
    assert conn.execute(
        'SELECT port_side, port_range, unique_port_count FROM port_count_stats '
        "WHERE src_visibility = 'literal' AND dst_visibility = 'anonymized' "
        'ORDER BY port_side'
    ).fetchall() == [('destination', 'high', 1), ('destination', 'low', 0), ('source', 'high', 0), ('source', 'low', 1)]
    assert conn.execute(
